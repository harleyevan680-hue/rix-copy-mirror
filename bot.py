...
from handlers.admin import panel,deposits,withdrawals,action,stats,is_admin
...
async def transfer_from(update, context):
    ...
    await q.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("◀️ Back",callback_data="transfer")]
        ])
    )
...
async def transfer_to(update, context):
    ...
    if not src or src==to:
        await q.edit_message_text("❌ Choose a different destination asset.",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back",callback_data="transfer")]]))
        return
    context.user_data["transfer_to"]=to
    context.user_data["transfer_stage"]="amount"
    await q.edit_message_text(f"🔄 TRANSFER {src} → {to}\n\nEnter the USD value you want to transfer.\n\n⚠️ This is an internal portfolio conversion; blockchain withdrawals are handled separately.",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel",callback_data="home")]]))
...
async def portfolio(update, context):
    ...
    await q.edit_message_text(
        f"📊 PORTFOLIO\n\n◎ SOL: {b['SOL']:.6f}\nΞ ETH: {b['ETH']:.6f}\n🟡 BNB: {b['BNB']:.6f}\n\n📂 Open Positions: {len(positions)}\n\n💵 Portfolio valuation requires live market pricing.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Refresh",callback_data="portfolio")],[InlineKeyboardButton("◀️ Back",callback_data="home")]])
    )
...
async def history(update, context):
    ...
    await q.edit_message_text(text,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back",callback_data="home")]]))
