...
async def _dashboard_text(uid):
    ...
    for asset in ('SOL','ETH','BNB'):
        try: prices[asset]=await get_usd_price(asset)
        except Exception: prices[asset]=0.0
    portfolio=sum(b[a]*prices.get(a,0.0) for a in b)
    ...
    return (
        ...
        f"◎ SOL Balance: {b['SOL']:.6f} SOL\n"
        f"Ξ ETH Balance: {b['ETH']:.6f} ETH\n"
        f"🟡 BNB Balance: {b['BNB']:.6f} BNB\n\n"
        f"💵 Portfolio Value: ${portfolio:,.4f}\n\n"
...
async def balance(update, context):
    ...
    from database import get_asset_balances, get_user_balance
    b=get_asset_balances(q.from_user.id)
    await q.edit_message_text(f"💳 YOUR BALANCE\n\n◎ SOL: {b['SOL']:.6f} SOL\nΞ ETH: {b['ETH']:.6f} ETH\n🟡 BNB: {b['BNB']:.6f} BNB\n\n💵 Trading Balance: ${get_user_balance(q.from_user.id):,.2f}",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Refresh",callback_data="balance")],[InlineKeyboardButton("◀️ Back",callback_data="home")]]))
...
async def sell_position(update, context):
    ...
    rows.append([InlineKeyboardButton("🔙 BACK",callback_data="sell_back")])
...
async def copy(update, context):
    ...
    if t and t["active"]:
        ...
        buttons=[[InlineKeyboardButton("🛑 STOP COPYING",callback_data="copy_stop")],[InlineKeyboardButton("🔙 BACK",callback_data="home")]]
    else:
        txt="🔁 COPY TRADING\n\n━━━━━━━━━━━━━━━━━━\n\nSelect a blockchain and enter the trader's PUBLIC wallet address.\n\n\n"
        buttons=[[InlineKeyboardButton("🚀 START COPY TRADING",callback_data="copy_start")],[InlineKeyboardButton("🔙 BACK",callback_data="home")]]
...
async def copy_stop(update, context):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔙 BACK", callback_data="copy")],
