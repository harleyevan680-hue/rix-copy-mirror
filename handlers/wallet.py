...
async def deposit(update, context):
    ...
    if not cfg:
        await q.edit_message_text("❌ Unsupported deposit asset.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 BACK", callback_data="wallet")]]))
        return
    if not address:
        await q.edit_message_text(
            ...
            "                   NOT READY                    ",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 BACK",callback_data="wallet")]]))
        return
    warning = {
