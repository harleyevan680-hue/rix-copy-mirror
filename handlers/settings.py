...
async def settings(update, context):
    ...
    await q.edit_message_text(settings_text(user, auto, copy), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back", callback_data="home")]]))
...
async def auto_enable(update, context):
    ...
    await q.edit_message_text("✅ AUTO TRADING ENABLED\n\nAuto trading is now enabled on SOL, ETH and BNB.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back", callback_data="auto")]]))
...
async def auto_disable(update, context):
    ...
    await q.edit_message_text("🛑 AUTO TRADING DISABLED\n\nAuto trading has been disabled on SOL, ETH and BNB.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back", callback_data="auto")]]))
...
async def auto_config(update, context):
    ...
    await q.edit_message_text("🤖 Configure Autotrade\n\nSelect the chain:", reply_markup=InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("◀️ Back", callback_data="auto")]
    ]))
...
async def auto_chain(update, context):
    ...
    await q.edit_message_text(f"🤖 Autotrade — {chain}\n\nEnter stop-loss % (e.g. 10 for 10%):", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel", callback_data="auto")]]))
...
async def auto_text(update, context):
    ...
    if stage=='sl':
        ...
        await update.message.reply_text(f"✅ Stop-loss: {value:.1f}%\n\nEnter take-profit % (e.g. 50):", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel", callback_data="auto")]])); return True
    if stage=='tp':
        context.user_data['auto_tp']=value; context.user_data['auto_stage']='max'
        await update.message.reply_text(f"✅ Take-profit: {value:.1f}%\n\nEnter max daily trades (e.g. 5):", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel", callback_data="auto")]])); return True
    if stage=='max':
